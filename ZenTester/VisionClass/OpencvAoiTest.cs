    ...
    public class OpencvAoiTest
    {
        ...
        public OpenCvSharp.Point CenterFineTopCamera(int index)
        {
            ...
            foreach (var contour in contours)
            {
                ...
                if (circularity > 0.55 && radius > 349  && radius < 1500) // 원형이고 일정 크기 이상 && area > 1000
                {
                    ...
                    if (radius > 800)
                    {
                        foreach (var point in contour)
                        {
                            ...
                            {
                                Cv2.Circle(colorView, new OpenCvSharp.Point(point.X, point.Y), 3, Scalar.Yellow, -1);
                            }
                            else
                            {
                                Cv2.Circle(colorView, new OpenCvSharp.Point(point.X, point.Y), 3, Scalar.Red, -1);
                            }
